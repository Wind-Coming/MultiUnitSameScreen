...
public class NW_TCPServer {
    ...
    public NW_TCPServer(int listenPort)
    ...
    public NW_TCPServer(IPEndPoint localEP)
    ...
    /// <param name="listenPort">监听的端口</param>
    public NW_TCPServer(IPAddress localIPAddress, int listenPort)
    {
    ...
    private void HandleDataReceived(IAsyncResult ar)
    {
        if (IsRunning) {
            ...
            state.MessageBuffer = new byte[recv];
            Buffer.BlockCopy(state.Buffer, 0, state.MessageBuffer, 0, recv);
            //触发数据收到事件
