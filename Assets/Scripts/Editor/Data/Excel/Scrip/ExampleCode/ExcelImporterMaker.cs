    ...
    static void ExportExcelToAssetbundle()
    {
        foreach (UnityEngine.Object obj in Selection.objects)
        {
			...
			using (FileStream stream = File.Open (window.filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                List<IDataValidation> allidv = sheet.GetDataValidations();

                IRow mt = sheet.GetRow(2);
                for( int i = 0; i < mt.LastCellNum; i ++ )
                {
                    Debug.Log(mt.GetCell(i).ToString());

                    for(int j = 0; j < allidv.Count; j ++ )
                    {
                        for(int m = 0; m < allidv[j].Regions.CellRangeAddresses.Length; m ++ )
                        {
                            if (allidv[j].Regions.CellRangeAddresses[m].IsInRange(2, i))
                            {
                                Debug.Log(mt.GetCell(i).ToString() + "hhhhhhhhhhhhhhhh");
                                {
                                    string all = allidv[j].ValidationConstraint.Formula1;
                                    all = all.Substring(1, all.Length - 2);
                                    string[] items = all.Split(',');
                                    string val = mt.GetCell(i).ToString();
                                    int result = 0;
                ...
                IComment ic = sheet.GetCellComment(0, 0);
